...
# fmt: off
# Adapted from https://github.com/flashinfer-ai/flashinfer/blob/main/flashinfer/cudnn/prefill.py
...
if CUDNN_AVAILABLE:
        ...
        with cudnn.graph(handle) as (g, _):
            ...
            if k_cache.dim() == 3:
                cudnn_k_cache = g.tensor(
                    ...
                    data_type=dtype_map[k_cache.dtype],
                )
                ...
                cudnn_v_cache = g.tensor(
                    ...
                    data_type=dtype_map[v_cache.dtype],
                )
            ...
            if cudnn.__version__ < '1.13.0':
                kwargs['is_inference'] = not kwargs['generate_stats']
                del kwargs['generate_stats']

            O, Stats = g.sdpa(**kwargs)
            ...
            ).set_data_type(dtype_map[q.dtype])
    ...
    # cuDNN not available - define stub function
    def _build_prefill_graph(*args, **kwargs):
        raise RuntimeError("cuDNN is not available. Please install cudnn package.")
